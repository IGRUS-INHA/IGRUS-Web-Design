...
- **Mobile-first 반응형**: 모바일 우선 설계 (Bootstrap 기준)
...
| md | 768~991px | 태블릿 세로 |
| lg | 992~1199px | 태블릿 가로 / 소형 데스크톱 |
| xl | 1200~1399px | 일반 데스크톱 |
| xxl | 1400px+ | 대형 모니터 |
...
> 평가 피드백에서 자주 사용되는 표현과 그에 대응하는 디자인 액션을 정의한다.
> 피드백에 아래 표현이 포함되면, 대응하는 액션을 우선 적용할 것.

### 피드백 패턴

피드백은 **"지금은 ___한데, ___했으면 좋겠어"** 형태로 주어진다.
앞부분은 현재 문제, 뒷부분은 목표 방향이다.

### 분위기 (Mood)

| 피드백 표현 | 반대 표현 | 디자인 액션 |
|---|---|---|
| 깔끔한 / 미니멀한 | 풍성한 / 화려한 | 장식 요소 줄이기, 여백 늘리기, 색상 수 제한 |
| 모던한 | 클래식한 / 전통적인 | 직선 레이아웃, 산세리프 강조, 플랫 컬러 |
| 따뜻한 / 친근한 | 차가운 / 전문적인 | 둥근 모서리(r3-r4), warm accent, 부드러운 그림자 |
| 가벼운 / 경쾌한 | 무게감 있는 / 묵직한 | font-weight 낮추기, 밝은 배경, 작은 요소 |
| 세련된 / 고급스러운 | 캐주얼한 / 편안한 | 넉넉한 여백, 얇은 테두리, 절제된 색상 사용 |

### 레이아웃 (Layout)

| 피드백 표현 | 디자인 액션 |
|---|---|
| 숨 쉴 공간이 없어 | padding/margin 증가 (최소 s5 이상), 섹션 간 간격 확대 |
| 붕 떠 있는 느낌 | max-width 줄이기, 그룹핑 강화, 배경/테두리로 영역 구분 |
| 다닥다닥 붙어 있어 | gap 증가, 카드 간 간격 확대 |
| 시선이 안 흘러가 | 정보 위계 재정렬, 시각적 흐름선 추가, CTA 위치 조정 |
| 위계가 안 느껴져 | 제목 크기/굵기 차이 확대, 색상 대비 강화, 여백으로 구분 |
| 덩어리감이 없어 | 카드나 섹션으로 그룹핑, 배경색 구분, 테두리 추가 |

### 타이포그래피 (Typography)

| 피드백 표현 | 디자인 액션 |
|---|---|
| 제목이 안 튀어 | font-weight 올리기(700→800), 크기 키우기, 색상 대비 강화 |
| 제목이 너무 튀어 | font-weight 내리기, 크기 줄이기, 색상 톤 다운 |
| 읽기 피곤해 | line-height 올리기(1.6+), 본문 크기 확인(최소 16px), 대비 확인 |
| 글씨가 답답해 | line-height 증가, letter-spacing 미세 조정, paragraph 간 간격 확대 |
| 폰트가 힘이 없어 | font-weight 올리기, 크기 키우기 |

### 색상 (Color)

| 피드백 표현 | 디자인 액션 |
|---|---|
| 톤이 안 맞아 | 색상 조합 재검토, brand palette 내에서 인접 토큰으로 교체 |
| 탁해 보여 | 채도 높은 토큰으로 교체, 배경 밝기 확인, 대비 강화 |
| 포인트 컬러가 묻혀 | primary 사용 면적 확대, 주변 색상 톤 다운 |
| 너무 단조로워 | accent 색상 추가, 그라데이션 활용, 상태별 색상 변화 |
| 너무 산만해 | 사용 색상 수 줄이기 (primary + gray 중심), 장식 색상 제거 |
| 대비가 약해 | WCAG AA 기준(4.5:1) 확인, foreground/background 토큰 재검토 |

### 감정·인상 (Impression)

| 피드백 표현 | 디자인 액션 |
|---|---|
| 신뢰감 있게 | 정돈된 그리드, 충분한 여백, 일관된 정렬, 절제된 색상 |
| 활동적인 느낌 | 강한 색 대비, 큰 이미지/히어로, bold CTA, 동적 요소 |
| 기술적인 느낌 | 모노스페이스 악센트, 다크 배경 활용, 그리드 패턴 |
| 커뮤니티 느낌 | 아바타/프로필 요소, 둥근 모서리(r3+), 따뜻한 accent |
| 스타트업 느낌 | 그라데이션 히어로, 큰 타이포, bold 카피, 넉넉한 여백 |

---

## 10. 프론트엔드 리드 선호 패턴 (평가 기반)
...
### 색상
- teal + gray 2톤만으로는 단조로움 — 섹션/카테고리별 accent 색상 변화 필요
- 바로가기 아이콘, 행사 카테고리 태그, 공지 배지 등에 다양한 색상 활용
- State Colors(Success, Warning, Info)나 brand palette 변주를 적극 활용하여 시각적 리듬감 부여
- 단, 전체 톤은 브랜드 아이덴티티(teal 기조) 안에서 유지할 것

### 히어로 스타일
- 인터랙티브 궤도 버블 히어로가 좋은 반응 (round-8 B)
- 2단 그리드(텍스트 + 비주얼) 레이아웃 선호
- 텍스트 순차 등장 애니메이션 긍정적

### 콘텐츠 섹션 스타일
- 타임라인 형태 행사 섹션 선호 (round-8 B)
- 영문 섹션 라벨(Notice, Events, Links) + 한글 제목 조합 선호
- 2단 분할(행사 + 바로가기) 레이아웃 선호
